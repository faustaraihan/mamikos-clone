...
import { getLocationById, type Room } from "@/data/local-data";
...
export function RoomListSection({
  ...
  const cities = [
    ...new Set(
      rooms
        .map((room) => getLocationById(room.locationId)?.city)
        .filter((value): value is string => Boolean(value)),
    ),
  ];
  const visible = rooms.filter(
    (room) => !city || getLocationById(room.locationId)?.city === city,
  );
  return (
        ...
        <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Combobox
              value={city || null}
              onValueChange={(value) =>
                setCity(typeof value === "string" ? value : "")
              }
            >
              <ComboboxTrigger
                aria-label={`Pilih kota ${title.toLowerCase()}`}
          ...
          <div className="flex items-center gap-4">
            <Button
              ...
              variant="outline"
              className="h-10 cursor-pointer rounded px-5 !bg-white hover:!bg-muted"
            >
              Lihat semua
            </Button>
        ...
        {visible.length ? (
          <CarouselContent className="-ml-6 py-1">
            {visible.map((room) => (
              <CarouselItem
                key={room.id}
                className="basis-[85%] pl-6 sm:basis-1/2 lg:basis-1/4"
              >
                <PromoRoomCard room={room} />
              </CarouselItem>
            ))}
          </CarouselContent>
        ) : (
          <p role="status" className="py-8 text-muted-foreground">
            Belum ada kos promo di kota ini.
          </p>
        )}
      </Carousel>
    </section>
